...
class firstSentence extends sentence {
    constructor(actions,names,objects) {
        const outline = (words) => `I love ${words[0]}ing ${words[1]}'s ${words[2]}`;
        super([actions,names,objects],outline);
    }
}

class secondSentence extends sentence {
    constructor(animals,adverbs,adjectives) {
        const outline = (words) => `${words[0]}s are ${words[1].trim()} ${words[2].trim()}`;
        super([animals,adverbs,adjectives],outline);
    }
}

class thridSentence extends sentence {
    //the cat pounces on the potato
    constructor(objects, verbs, objects2) {
        const outline = (words) => `the ${words[0]} ${words[1]}s on the ${words[2]}`
        super([objects,verbs,objects2],outline);
    }
}
